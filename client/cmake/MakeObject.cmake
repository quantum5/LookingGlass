function(make_single_object in_f sym_in out_f out_h)
  string(REGEX REPLACE "[/.]" "_" ld_sym  ${in_f})

  add_custom_command(OUTPUT ${out_f}
    COMMAND ${CMAKE_LINKER} -r -b binary -o ${out_f} ${in_f}
    COMMAND ${CMAKE_OBJCOPY} --rename-section .data=.rodata,CONTENTS,ALLOC,LOAD,READONLY,DATA ${out_f} ${out_f}
    COMMAND ${CMAKE_OBJCOPY} --redefine-sym _binary_${ld_sym}_start=b_${sym_in} ${out_f} ${out_f}
    COMMAND ${CMAKE_OBJCOPY} --redefine-sym _binary_${ld_sym}_end=b_${sym_in}_end ${out_f} ${out_f}
    COMMAND ${CMAKE_OBJCOPY} --strip-symbol _binary_${ld_sym}_size ${out_f} ${out_f}
    DEPENDS ${in_f}
...
function(make_object out_var)
  ...
  foreach(in_f ${ARGN})
    ...
    make_single_object("${in_f}" "${sym_in}" "${out_f}" "${out_h}")
